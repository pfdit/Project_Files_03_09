...
public class CustomerOrderTab extends JPanel implements ActionListener, ItemListener, ListSelectionListener, ComponentListener{
	...
	public CustomerOrderTab(Staff currentlyLoggedInStaff, PersonDB personDB, StockDBControl stockDBControl, OrderDB orderDB, MainGUI gui){
	...
	private void refreshTab(int index){		
		...
		if(customerComboBox.getSelectedIndex()!=0){
			customerIDField.setText(Integer.toString(personDB.getCustomerList().get((customerComboBox.getSelectedIndex()-1)).getId()));
		}
	}
	...
	public void itemStateChanged(ItemEvent event) {
		...
		if (event.getItemSelectable().equals(customerComboBox)) {
			...
			/*else
				customer = personDB.getSupplierByName(supplierComboBox
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource()==processButton){
			...
			Customer customer = (Customer)personDB.getCustomerList().get(customerComboBox.getSelectedIndex()-1);
			...
			*/
			...
			for (int index = 0; index < currentOrderList.size(); index++){
				quantity = stockDBControl.getStockItem(currentOrderList.get(index).getProductID()).getQuantity();				
				quantity -= currentOrderListQuantity.get(index);			
				stockDBControl.getStockItem(currentOrderList.get(index).getProductID()).setQuantity(quantity);				
			}
			
			gui.getCustomerTab().setAutomaticItemSelection(true);
			gui.getCustomerTab().addItemsToOrderCombobox();
			gui.getCustomerOrderHistorytab().setOrderList();
			showOrderDetails(order);
		
		
		/*JOptionPane.showMessageDialog(null,"Order created by " + currentlyLoggedInStaff.getName() +
	...
	private void automaticallyCreateOrders(){
		...
		for(int i=0;i<random.nextInt((10 - 5) + 1) + 5;i++){
			orderDB.getCustomerOrderList().add(new Order(personDB.getRandomStaff(), (Customer)personDB.getRandomCustomer(), getRandomOrderItemList(true), grandTotal));
		}
		...
		for(ArrayList<Product> list : productList){
			...
			if(orderStockItemList.size()>0)
				orderDB.getSupplyOrderList().add(new Order(personDB.getRandomStaff(), orderStockItemList.get(orderStockItemList.size()-1).getProduct().getSupplier(), orderStockItemList, grandTotal));
			else
	...
	private void showOrders(){
		...
		for(int i=0;i<orderDB.getSupplyOrderList().size();i++){
			...
			System.out.println("Order "+order.getId()+" was created at "+order.getDate()+" by "+order.getCurrentlyLoggedInStaff().getName());
			...
			for(int j=0;j<order.getOrderEntryList().size();j++){
				...
				System.out.println("  "+stockItem.getQuantity()+"  "+stockItem.getProduct().getProductName()+"  "+(stockItem.getProduct().getRetailPrice()*stockItem.getQuantity()));
			}
