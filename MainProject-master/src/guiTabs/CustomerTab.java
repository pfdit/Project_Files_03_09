...
/*
	...
	public void personDetailsForm(){	
		...
		if(name!=null && address!=null && email!=null && contactNumber!=null){
			...
			if(editMode){
				...
				setTextField(getIndex(personDB.getCustomerList()), personDB.getCustomerList());
				valid = true;
			}
			else{
				valid = true;
		...
		if(e.getSource()==submitButton){
			if(submitButtonMode == 3){
				...
				if(valid){
					...
					((JTabbedPane)customerOrderTab.getParent()).setSelectedComponent(customerOrderTab);
					customerOrderTab.fillUpCustomerComboBox();
					customerOrderTab.getCustomerComboBox().setSelectedIndex(personDB.getCustomerList().size());
				}
			}
	...
	public void itemStateChanged(ItemEvent event) {
		...
		if (event.getStateChange() == ItemEvent.SELECTED) {
			if (event.getItemSelectable().equals(comboBox)) {
				...
				if(getIndex(personDB.getCustomerList())!=-1){
					//JOptionPane.showMessageDialog(null, getIndex());
					setTextField(getIndex(personDB.getCustomerList()),personDB.getCustomerList());	
				}
	        }
			if (event.getItemSelectable().equals(orderComboBox)) {
				...
				if(values!=null&&!automaticItemSelection){
					showOrderDetails(customerOrderTab.getOrderDB().getOrderById(getIndex(personDB.getCustomerList()), customerOrderTab.getOrderDB().getCustomerOrderList()));
					//automaticItemSelection = true;
				}
			}
			revalidate();
	...
	public void mouseClicked(MouseEvent arg0) {
		if(orderComboBox.isEnabled()){
			String[] values = ((String)orderComboBox.getSelectedItem()).split("\\t");
			if(values!=null){
				showOrderDetails(customerOrderTab.getOrderDB().getOrderById(Integer.parseInt(values[1].trim()), customerOrderTab.getOrderDB().getCustomerOrderList()));
			}	
		}
	}
